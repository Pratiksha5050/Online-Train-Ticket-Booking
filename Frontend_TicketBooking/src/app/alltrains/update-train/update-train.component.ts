...
export class UpdateTrainComponent implements OnInit{
  ...
  dateFilter: DateFilterFn<Date | null> = (date: Date | null) => {
    // Example filter logic: allow dates in the future
    if (!date) {
      // Return true to allow null values (no date selected)
      return true;
    }
  ...
  // Example event handler for date changes
  onDateChange(event: MatDatepickerInputEvent<Date>) {
    console.log('Selected date:', event.value);
  }

  // Example event handler for date picker opening
  onPickerOpened() {
  ...
  ngOnInit(): void {
    ...
    this.updateForm = this.fb.group({
      ...
      dates:[this.data.train.dates,Validators.required],
  ...
  {
    return this.updateForm.get('seatsInSecondClassClass');
  }
