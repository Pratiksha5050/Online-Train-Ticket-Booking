...
public class TrainController {
	...
	{
		Train t= trainInterface.getTrainOfBooking(trainNo);
      
        if (t == null) {
            throw new TrainNotFoundException(trainNo);
        }
        return t;
	}
...
//	}
    ...
    public Train updateTrain(@PathVariable String trainNo, @RequestBody Train updatedTrain) {		
...
//	}
	...
	public ResponseEntity<String> getSeatsByID(@PathVariable String trainNo) {
		Train train = trainInterface.findById(trainNo).orElseThrow(() -> new TrainNotFoundException(trainNo));
		int a = train.getSeatsInFirstClassClass()+train.getSeatsInSecondClassClass()+train.getSeatsInSleeperClass()+train.getSeatsInThirdClassClass();
